...
function createActor(spec) {
  ...
  spec.name   = help.define(spec.name).or.use(defaults.actor.name);
  spec.pools  = help.define(spec.pools).or.use(defaults.actor.pools);

  for (let key in spec.pools) {
    let data = spec.pools[key];

    spec.pools[key] = pool(data);
  }
